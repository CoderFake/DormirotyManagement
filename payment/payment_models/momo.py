...
class MomoTransaction(models.Model):
    ...
    user = models.ForeignKey('accounts.User', on_delete=models.CASCADE, related_name='momo_transactions',
                             verbose_name=_('người dùng'))
    payment = models.OneToOneField('payment.Payment', on_delete=models.CASCADE, related_name='momo_transaction',
    ...
    order_id = models.CharField(_('mã đơn hàng'), max_length=100, unique=True)
    request_id = models.CharField(_('mã yêu cầu'), max_length=100, unique=True)
    transaction_id = models.CharField(_('mã giao dịch Momo'), max_length=100, blank=True, null=True)
    ...
    class Meta:
        verbose_name = _('giao dịch Momo')
        verbose_name_plural = _('giao dịch Momo')
        ordering = ['-created_at']
